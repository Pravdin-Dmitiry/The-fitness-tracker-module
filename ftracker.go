...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speed := math.Pow((distance(action) / duration), 2)
	calories = ((walkingCaloriesWeightMultiplier*weight + (speed/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
